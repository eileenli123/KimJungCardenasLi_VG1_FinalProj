# "Get that Degree" Unity Game

## Overview  
**[Get that Degree]** is a thrilling Unity-based game where players navigate through a series of challenging levels of college years to graduate. The goal is to finish all levels while optimizing gameplay stats to maximize the final “score” determined by the player’s career outcome, major, gpa, and social success.  

## Play Now  
You can play the game online directly using this link:  
>>[Play **[Get that Degree]** on itch.io](https://jmijung.itch.io/wow?secret=Cd41YCG72QbxK6RyHNVUolsfL0)  

## Gameplay Mechanics  
- **Objective**: Reach the end of each level and successfully complete all levels to graduate and win. There are 5 levels in total: 1. Orientation Week, 2. Freshman Year, 3. Sophomore Year, 4. Junior Year, 5. Senior Year. Upon finishing each level, the player has to go through a checkpoint where the player can choose a major, and purchase social or academic gems using coins they earned during the game.

- **Collectibles**:  
  - **GPA Gems**: Boost your academic performance.  
  - **Social Gems**: Enhance your social life balance.
  - **Coin Gems**: Allow purchasing GPA or Social Gems at every checkpoint.

- **Scoring**:  
  - Your final score is based on your gameplay stats:  
    - **Internship Success**: Determined by the choice of your internship.  
    - **Major**: Determined by your major choice. The player gets rewarded more if they choose a major that requires higher GPA.
    - **Collectibles**: Leftover GPA/Social gems are transferred to an additional score.
      
## Screenshot  
<img width="500" alt="orientation week" src="https://github.com/user-attachments/assets/70ec57a1-5faf-46ab-be34-1bf8cda4b7b3" />
*Figure 1: Orientation Week Level*

<img width="500" alt="freshman" src="https://github.com/user-attachments/assets/405ee26f-456c-4b9a-9735-a565d8375a6d" />
*Figure 2: Freshman Year Level*

<img width="500" alt="sophomore" src="https://github.com/user-attachments/assets/b0c9e859-4f64-4b0d-be3c-754b8f371fb5" />
*Figure 3: Sophomore Year Level*

<img width="500" alt="junior" src="https://github.com/user-attachments/assets/7972813a-3886-49c4-b7b2-c37c56298b93" />
*Figure 4: Junior Year Level*

<img width="500" alt="senior" src="https://github.com/user-attachments/assets/26a77665-1f06-4b3f-b76d-2fc21ab1d496" />
*Figure 5: Senior Year Level*

<img width="500" alt="checkpoint" src="https://github.com/user-attachments/assets/4e230391-6fe8-4176-87eb-072cc29ebfbd" />
*Figure 6: Checkpoint*

<img width="500" alt="score" src="https://github.com/user-attachments/assets/1f11bd67-9a30-444d-a1d0-2218962bdb63" />
*Figure 7: Score Board Example*

## Features  
- **Challenging Levels**: Navigate diverse levels with unique obstacles: attending classes to obtain social gems, going on a date, talking with friends, maze, etc.
- **Interactive Tutorial**: Orientation week has an interactive tutorial to allow players to understand the game better.
- **Stat-Based Outcomes**: Your performance directly impacts your post-graduation achievements.  
- **Immersive Design**: Engaging visuals and intuitive controls.  
- **Replay Value**: Try different strategies to achieve the best score and unlock various endings.  

## Credits  
- Background Music by <a href="https://pixabay.com/users/cyberwave-orchestra-23801316/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=249202">Cyberwave Orchestra</a> from <a href="https://pixabay.com//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=249202">Pixabay</a>
- Sound Effect by <a href="https://pixabay.com/users/freesound_community-46691455/?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=6075">freesound_community</a> from <a href="https://pixabay.com/sound-effects//?utm_source=link-attribution&utm_medium=referral&utm_campaign=music&utm_content=6075">Pixabay</a>
- Sound Effect by https://www.fiftysounds.com/royalty-free-music/sfx-defeat2.html
